...
internal class CustomSqsBatchProcessor : SqsBatchProcessor
{
        ...
        IRecordHandler<SQSEvent.SQSMessage> recordHandler, ProcessingOptions processingOptions)
    {
        ...
        var batchRecords = GetRecordsFromEvent(@event).Select(x => new KeyValuePair<string, SQSEvent.SQSMessage>(GetRecordId(x), x))
            .ToArray();

        // We assume all records fail by default to avoid loss of data
        var failureBatchRecords = batchRecords.Select(x => new KeyValuePair<string, RecordFailure<SQSEvent.SQSMessage>>(x.Key,
            new RecordFailure<SQSEvent.SQSMessage>
            {
        ...
        var successRecords = new Dictionary<string, RecordSuccess<SQSEvent.SQSMessage>>();
        var failureRecords = new Dictionary<string, RecordFailure<SQSEvent.SQSMessage>>(failureBatchRecords);
        ...
        try
        {
            foreach (var pair in batchRecords)
            {
                ...
                try
                {
                    ...
                    successRecords.TryAdd(recordId, new RecordSuccess<SQSEvent.SQSMessage>
                    {
                        Record = record,
                        RecordId = recordId,
                ...
                {
                    // Capture exception
                    failureRecords[recordId] = new RecordFailure<SQSEvent.SQSMessage>
                    {
...
public class BadCustomSqsRecordProcessor
{
}