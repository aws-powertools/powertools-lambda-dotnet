...
```bash
Tracing$ sam local invoke HelloWorldFunction --event events/event.json
```

The AWS SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
Tracing$ sam local start-api
Tracing$ curl http://localhost:3000/
```
...
```bash
Tracing$ sam logs -n HelloWorldFunction --stack-name Logging --tail
```
...
```bash
Tracing$ dotnet test test/HelloWorld.Test
```
