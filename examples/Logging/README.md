...
```bash
Logging$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
Logging$ sam local start-api
Logging$ curl http://localhost:3000/
```
...
```bash
Logging$ sam logs -n HelloWorldFunction --stack-name Logging --tail
```
...
```bash
Logging$ dotnet test test/HelloWorld.Test
```
...
```bash
aws cloudformation delete-stack --stack-name Logging
```
