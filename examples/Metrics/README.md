...
```bash
Metrics$ sam local invoke HelloWorldFunction --event events/event.json
```

The AWS SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
Metrics$ sam local start-api
Metrics$ curl http://localhost:3000/
```
...
```bash
Metrics$ sam logs -n HelloWorldFunction --stack-name Logging --tail
```
...
```bash
Metrics$ dotnet test test/HelloWorld.Test
```
