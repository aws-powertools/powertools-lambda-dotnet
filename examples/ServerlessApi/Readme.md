...
This project contains source code and supporting files for a web api hosted on Lambda that you can deploy with the AWS Serverless Application Model Command Line Interface (AWS SAM CLI). It includes the following files and folders.
...
The application uses several AWS resources, including Lambda functions and an API Gateway API. The project has been created using `Lambda ASP.NET Core Web API` [.NET Core project templates](https://github.com/aws/aws-lambda-dotnet/tree/master#amazonlambdaaspnetcoreserver). Events folder contains example API Gateway proxy Lambda events and are not offered by the project template.
...
* **AWS Region**: The AWS region you want to deploy your app to. Rest of the document will assume that stack name is `eu-central-1`
...
To simplify troubleshooting, AWS SAM CLI has a command called `sam traces`. `sam traces` lets you fetch traces generated by your deployed Api Gateway and Lambda and Api function from the command line. To see the trace for the given   In addition to printing the logs on the terminal, this command has several nifty features to help you quickly find the bug.
